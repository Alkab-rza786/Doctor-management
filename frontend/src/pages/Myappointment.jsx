        ...
        {appointments.slice(0, 3).map((item, i) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={i}>
            ...
            <div className='flex flex-col gap-2 justify-end '>
              ...
              {!item.cancelled && !item.payment && <button onClick={() => appointmentRazorpay(item._id)} className='text-sm to-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-300' >Pay Online</button>}
              {!item.cancelled && <button onClick={() => cancelAppointment(item._id)} className='text-sm to-stone-500 text-center sm:min-w-48 py-2 border rounded   hover:bg-red-600 hover:text-white transition-all duration-300 ' >Cancel Appoinment</button>}
